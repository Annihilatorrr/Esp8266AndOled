                     ...
                     const char* utf8)
{
  ...
  while (*p && lineIdx < maxLines) {
    ...
    if (test[0] != '\0') appendChar(test, sizeof(test), ' ');
    ...
    if (px <= w) {
      ...
      if (u8g2.getUTF8Width(word) > w) {
        ...
        while (*q) {
          ...
          if (u8g2.getUTF8Width(tmp) > w) break;
          strcpy(chunk, tmp);
          q++;
        }
