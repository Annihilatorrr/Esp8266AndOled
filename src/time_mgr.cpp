...
TimeMgr::TimeMgr()
	: synced_(false), lastFetchMs_(0), lastTime_("--:--:--"), lastDate_("----------") {}
...
void TimeMgr::update() {
		...
		if (synced_ && (int32_t)(now - lastFetchMs_) < 0) return;
		if (synced_ && (now - lastFetchMs_ < kFetchIntervalMs)) return;
		// throttle rapid retries to once every 5 seconds
		if (!synced_ && (now - lastFetchMs_ < 5000)) return;
		lastFetchMs_ = now;

	// Use a secure client for HTTPS; accept any cert for simplicity (not for production)
	...
	if (code == HTTP_CODE_OK) {
		...
		if (p >= 0) {
			...
			// parse digits
			int q = p;
			while (q < payload.length() && isDigit(payload.charAt(q))) q++;
			if (q > p) {
			...
			if (q >= 0) {
				...
				if (q + kDateTimeTotalLen <= payload.length()) {
					...
					if (dt.length() >= kDateTimeTotalLen && dt.charAt(kDateTimeTPos) == 'T') {
						...
						// compute days since epoch using civil_from_days algorithm
						...
						// apply utc_offset if available (offsetSeconds parsed earlier)
						...
						synced_ = true;
						lastFetchMs_ = now;
						Serial.printf("[Time] fallback datetime=%s -> epoch=%lu (offsetApplied=%d)\n", dt.c_str(), epoch, offsetSeconds);
					}
				}
			}
		}
	...
	if (!synced_) {
		...
		while (millis() - start < 1500) {
			...
			if (len >= 48) {
				...
				uint8_t buf[48];
				sntpUdp.read(buf, 48);
				unsigned long sec = ((unsigned long)buf[40] << 24) | ((unsigned long)buf[41] << 16) | ((unsigned long)buf[42] << 8) | ((unsigned long)buf[43]);
...
String TimeMgr::timeString() const {
	if (!synced_ || lastEpochLocal_ == 0) return lastTime_;
	unsigned long now = millis();
	unsigned long cur = lastEpochLocal_ + ((now - lastFetchMs_) / 1000UL);
	time_t t = (time_t)cur;
	struct tm *tm = gmtime(&t);
	if (!tm) return lastTime_;
	char buf[16];
	snprintf(buf, sizeof(buf), "%02d:%02d:%02d", tm->tm_hour, tm->tm_min, tm->tm_sec);
	return String(buf);
}
...
String TimeMgr::dateString() const {
	if (!synced_ || lastEpochLocal_ == 0) return lastDate_;
	unsigned long now = millis();
	unsigned long cur = lastEpochLocal_ + ((now - lastFetchMs_) / 1000UL);
	time_t t = (time_t)cur;
	struct tm *tm = gmtime(&t);
	if (!tm) return lastDate_;
	char buf[16];
	snprintf(buf, sizeof(buf), "%04d-%02d-%02d", tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday);
	return String(buf);
}
